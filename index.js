...
function processSum(numberList, callback) {
  const num = (acc, curr) =>  acc + curr;
  return callback(numberList.reduce(num,0));
}
...
function getFullNames(runners) {
  ...
  runners.forEach((value) => {
      return fullName.push(`${value.last_name}, ${value.first_name}`)
  }
  )
      return fullName};
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const size = runners.filter((person) => {
